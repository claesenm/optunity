function [ in, out, p, socket ] = optunity_comm_popen( cmd, env )
%POPEN Spawns a subprocess p via Java API and enables bidirectional
%communication via sockets.

%% create server socket
%serverSocket = java.net.ServerSocket(0);
serverSocket = javaObject("java.net.ServerSocket", 0);

port = serverSocket.getLocalPort();

% append port number to optunity_s launch command
cmd = [cmd, ' ', num2str(port)];

%% launch Optunity Python back-end
%rt = java.lang.Runtime.getRuntime();
rt = javaMethod("java.lang.Runtime.getRuntime")
if numel(env) > 0
    if iscell(env)
        dim = numel(env);
